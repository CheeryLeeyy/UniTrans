...
def train_parser():
    ...
    parser.add_argument("--pth_name", type=str, default="merged.pth")
...
def main():
    ...
    hypes["model"]["args"]["model_dir"] = opt.model_dir
    ...
    #     saved_path = pretrained_dir
    #     pretrained_epoch, model = train_utils.load_saved_model_safe(saved_path, model)
    #     print(f"Loaded pretrained model from {saved_path}")
    #     init_epoch = 0
    #
    #     # Determine training directory
    #     if resume_dir:
    #         saved_path = resume_dir
    #     else:
    ...
    if opt.model_dir:
        ...
        init_epoch, model = train_utils.load_saved_model_safe(saved_path, model)
        if init_epoch == 0:
            model = train_utils.load_pretrained_model(saved_path, model, opt.pth_name)
        lowest_val_epoch = init_epoch
    ...
    if init_epoch <= 1:
        try:
            ...
            print("No init_parameters() function")
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    final_loss = criterion(ouput_dict, batch_data['ego']['label_dict'], val=True)
    ...
    if opt.rank == 0:
        ...
        if len(bestval_model_list) > 1:
            import numpy as np
            bestval_model_epoch_list = [eval(x.split("/")[-1].lstrip("net_epoch_bestval_at").rstrip(".pth")) for x in
                                        bestval_model_list]
    ...
    # Prepare test command, defer execution to after main() returns
    test_cmd = None
    # if opt.rank == 0:
    #     fusion_method = opt.fusion_method
    #     test_cmd = f"python opencood/tools/inference_0322.py  --model_dir {saved_path} --fusion_method {fusion_method}"
    #     print(f"Prepared test command: {test_cmd}")
    # return test_cmd
...
if __name__ == '__main__':
    cmd = main()
    # if cmd:
    #     print(f"Running test command after cleanup: {cmd}")
    #     os.system(cmd)
