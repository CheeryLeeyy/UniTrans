...
def test_parser():
    ...
    parser.add_argument("--save_path_suffix", default="", type=str, help="vis save path suffix under model dir")
    parser.add_argument("--vis_color_method", default="modality", type=str, help="Visual point cloud color assignment method")
    parser.add_argument('--save_features', action='store_true', help="Whether to save intermediate feature")
    parser.add_argument('--feature_output_folder', type=str, default="", help="Where to save intermediate feature")
    parser.add_argument('--save_features_dataset', type=str, default="train", help="dataset to save intermediate feature")
...
def main():
    ...
    # for saving intermediate feature
    if opt.save_features:
        hypes['model']['args']['save_features'] = True
        hypes['model']['args']['output_folder'] = opt.feature_output_folder
        last_slash_idx = hypes['validate_dir'].rfind("/")
        hypes['validate_dir'] = hypes['validate_dir'][:last_slash_idx + 1] + opt.save_features_dataset
...
def eval_detection_result(
    ...
    if (i % opt.save_vis_interval == 0) and (pred_box_tensor is not None or gt_box_tensor is not None):
        vis_save_path_root = os.path.join(work_dir, f'vis_{opt.infer_info}{"_protocol" if opt.protocol_result else ""}{opt.save_path_suffix}')
        color_method='modality'
        if opt.vis_color_method:
            color_method=opt.vis_color_method
        ...
        #     pcd_cav=batch_data["ego"]["origin_lidar_cav_id"][0],
