...
def is_multiple_agents(output_dict):
    ...
    old format is {pred}
    """
    return [(key[0] == 'm' and key[1:].isdigit()) for key in output_dict.keys()].count(True) != 0
...
def inference_early_fusion(batch_data, model, dataset, show_bev=False, protocol_result=False):
    ...
    if "feat_vis" in output_dict["ego"]:
        return_dict.update({"feat_vis": output_dict["ego"]["feat_vis"]})
