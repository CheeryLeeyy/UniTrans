...
class HomoCollab(nn.Module):
    ...
    def forward(self, data_dict, show_bev=False):
        ...
        if modality_name in modality_count_dict:
            ...
            # print(f"spatial_features_2d:{feature.shape}") # 64, 128, 256
            feature = eval(f"self.aligner_{modality_name}")(feature)

            # save intermediate feature
            if self.save_feature_flag:
                self.save_feature(feature, data_dict)


            # ---------------------------
            # 打印SECOND模型的参数
            # ---------------------------
            # print("\n[SECOND Encoder Parameters]")
            # total_params = 0
            # for name, param in self.encoder_m2.named_parameters():
            #     # if param.requires_grad:
            #     param_shape = list(param.shape)
            #     num_params = param.numel()
            #     total_params += num_params
            #     print(f"{name}: shape={param_shape}, params={num_params}")
            #
            # print(f"SECOND Total Trainable Params: {total_params / 1e6:.2f}M")
            # print("\n[PointPillar Encoder Parameters]")
            # total_params = 0
            # for name, param in self.encoder_m6.named_parameters():
            #     # if param.requires_grad:
            #     param_shape = list(param.shape)
            #     num_params = param.numel()
            #     total_params += num_params
            #     print(f"{name}: shape={param_shape}, params={num_params}")
            #
            # print(f"PointPillar Total Trainable Params: {total_params / 1e6:.2f}M")
            # print("\n[Backbone Encoder Parameters]")
            # total_params = 0
            # for name, param in self.backbone_m4.named_parameters():
            #     # if param.requires_grad:
            #     param_shape = list(param.shape)
            #     num_params = param.numel()
            #     total_params += num_params
            #     print(f"{name}: shape={param_shape}, params={num_params}")
            #
            # print(f"Backbone Total Trainable Params: {total_params / 1e6:.2f}M")
            # print("\n[Pyramid fusion Encoder Parameters]")
            # total_params = 0
            # for name, param in self.pyramid_backbone_m4.named_parameters():
            #     # if param.requires_grad:
            #     param_shape = list(param.shape)
            #     num_params = param.numel()
            #     total_params += num_params
            #     print(f"{name}: shape={param_shape}, params={num_params}")
            #
            # print(f"Pyramid fusion Total Trainable Params: {total_params / 1e6:.2f}M")
        ...
        # print(f"after encoder and backbone.shape:{heter_feature_2d.shape}")
