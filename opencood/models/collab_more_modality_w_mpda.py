...
class CollabMoreModalityWMPDA(nn.Module):

    def __init__(self, args):
        super(CollabMoreModalityWMPDA, self).__init__()
        ...
        self.unfix_modules = [self.resizer, self.cdt, self.classifier]
    ...
    def forward(self, data_dict, show_bev=False):
        ...
        for i in range(len(heter_feature_split)):
            ...
            _, _, H, W = heter_feature_split[i].shape
            ...
            if heter_feature_split[i].shape[0] == 1:  # only ego
                ...
                cav_feature = self.resizer(ego_feature, cav_feature)
                ## 将ego特征转换到neb坐标系下
                t_matrix = affine_matrix[i][:record_len[i], :record_len[i], :, :]
                ego_repeat = ego_feature.repeat(heter_feature_split[i].shape[0], 1, 1, 1)
                ego_copy = warp_affine_simple(ego_repeat, t_matrix[:, 0, :, :], (H, W), align_corners=True)[1:]
        ...
        heter_feature_2d_list = torch.cat(reshaped_spatial_feature_list, dim=0)
        ...
        output_dict.update({
            'da_feature': class_logits, 'record_len_da': record_len,
            "feat_vis": {
