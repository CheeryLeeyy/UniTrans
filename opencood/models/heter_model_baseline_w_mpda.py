...
class HeterModelBaselineWMPDA(nn.Module):
    def __init__(self, args):
        ...
        for modality_name in self.modality_name_list:
            ...
            if model_setting['backbone_args'] == 'identity':
                ...
                setattr(self, f"backbone_{modality_name}", BaseBEVBackbone(model_setting['backbone_args'],
                                                                           model_setting['backbone_args'].get(
                                                                               'inplanes', 64)))
            ...
            if self.spatial_shrinker_flag:
                setattr(self, f'spatial_shrinker_{modality_name}',
                        nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1))
                setattr(self, f'spatial_deshrinker_{modality_name}',
        ...
        if args.get("supervise_single", False):
            ...
            setattr(self, f'cls_head_single',
                    nn.Conv2d(in_head_single, args['anchor_number'] * self.num_class * self.num_class, kernel_size=1))
            setattr(self, f'reg_head_single',
                    nn.Conv2d(in_head_single, args['anchor_number'] * 7 * self.num_class, kernel_size=1))
            setattr(self, f'dir_head_single',
                    nn.Conv2d(in_head_single, args['anchor_number'] * args['dir_args']['num_bins'], kernel_size=1))
    ...
    def forward(self, data_dict):
        ...
        if not self.training and self.missing_message:  # for missing_massage inference
            ...
            for i in range(1, heter_feature_2d.shape[0]):
                mask = torch.rand(heter_feature_2d.shape[1], heter_feature_2d.shape[2], heter_feature_2d.shape[3],
                                  device=heter_feature_2d.device) > missing_level
        ...
        if self.spatial_shrinker_flag:
            ...
            for i in range(len(agent_modality_list)):
                ...
                heter_feature_list.append(
                    eval(f"self.spatial_deshrinker_{modality_name}")(heter_feature_2d[i].unsqueeze(0)))
