...
class CamEncode_Resnet34(CamEncode_Resnet101):  # 提取图像特征进行图像编码
    def __init__(self, D, C, downsample, ddiscr, mode, use_gt_depth=False, depth_supervision=True):
        ...
        if not use_gt_depth:
            self.depth_head = nn.Conv2d(128, self.D, kernel_size=1, padding=0)  # 1x1卷积，变换维度

        self.image_head = nn.Conv2d(128, self.C, kernel_size=1, padding=0)
...
class CamEncode_EfficientNet_B2(CamEncode):  # 提取图像特征进行图像编码
    def __init__(self, D, C, downsample, ddiscr, mode, use_gt_depth=False, depth_supervision=True):
        ...
        # self.up1 = Up(320+112, 512)  # 上采样模块，输入输出通道分别为320+112和512
        self.up1 = Up(472, 512)  # 上采样模块，输入输出通道分别为472和512
        if downsample == 8:
            # self.up2 = Up(512+40, 512)
            self.up2 = Up(560, 512)
        if not use_gt_depth:
