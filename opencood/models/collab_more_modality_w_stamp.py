...
class CollabMoreModalityWStamp(nn.Module):

    def __init__(self, args):
        super(CollabMoreModalityWStamp, self).__init__()

        self.adapter = Adapter(args["adapter"])
        self.reverter = Reverter(args["reverter"])
        self.protocol_modality = args["protocol_modality"]

        self.unfix_modules = [self.adapter, self.reverter]
    ...
    def forward(self, data_dict, show_bev=False):
        ...
        with torch.no_grad():  #
            ...
            if not self.testing:
                ...
                # self.protocol_modality
                ########

                feature_p = self.forward_encoder(data_dict, "protocol", output_dict)
                # import pdb; pdb.set_trace()
                # print(f'spatial_feature,shape:{feature.shape}')
                feature_p = self.forward_backbone(feature_p, self.protocol_modality)
                # max_val = torch.max(feature)
                # min_val = torch.min(feature)
                # cv2.imwrite("debug/feature.png", ((feature[0].abs().max(0)[0].cpu().numpy() - feature[0].abs().max(0)[0].cpu().numpy().max()) / (feature[0].abs().max(0)[0].cpu().numpy().max() - feature[0].abs().max(0)[0].cpu().numpy().min()) * 255).astype(np.uint8))
                # print(f'spatial_feature_2d.shape:{feature.shape}')
                FP = self.forward_aligner(feature_p, self.protocol_modality)

                if not eval(f"self.multi_sensor_{self.protocol_modality}"):
                    """
                    Crop/Padd camera feature map.
                    """
                    if "camera" in self.sensor_type_dict[self.protocol_modality]:
                        feature = FP
                        _, _, H, W = feature.shape
                        target_H = int(H * eval(f"self.crop_ratio_H_{self.protocol_modality}"))
                        target_W = int(W * eval(f"self.crop_ratio_W_{self.protocol_modality}"))

                        crop_func = torchvision.transforms.CenterCrop((target_H, target_W))
                        FP = crop_func(feature)
                        if eval(f"self.depth_supervision_{self.protocol_modality}"):
                            output_dict[self.protocol_modality].update(
                                {f"depth_items_{self.protocol_modality}": eval(
                                    f"self.encoder_{self.protocol_modality}").depth_items}
                            )
        ...
        heter_feature_2d_list = []
        FN2P_feature_2d_list = []
        FN2P2E_feature_2d_list = []

        ego_in_nebcoord_list = []
        ...
        for mini_batch_idx, mini_batch_size in enumerate(record_len):
            ...
            _FN2P = self.adapter(batch_feature)
            _FN2P2E = self.reverter(torch.cat([_FN2P, ego_in_nebcoord], dim=1))

            FN2P_feature_2d_list.append( _FN2P )
            FN2P2E_feature_2d_list.append( _FN2P2E )
            ego_in_nebcoord_list.append( ego_in_nebcoord  )

            batch_feature_output = _FN2P2E
        ...
        FN2P_feature_2d_list = torch.cat(FN2P_feature_2d_list, dim=0)
        FN2P2E_feature_2d_list = torch.cat(FN2P2E_feature_2d_list, dim=0)
        ego_in_nebcoord_list = torch.cat(ego_in_nebcoord_list, dim=0)

        F_P2E = None
        if not self.testing:
            P2E_inputs = torch.cat([output_dict["FP"], ego_in_nebcoord_list], dim=1  )
            F_P2E = self.reverter(P2E_inputs)
        output_dict.update({
            "FN2E": heter_feature_2d_list,
            "FN2P2E": FN2P2E_feature_2d_list,
            "FP2E": F_P2E,
            "FN2P": FN2P_feature_2d_list,
            "feat_vis": {
        ...
        all_processed_features = heter_feature_2d_list
    ...
    def forward_encoder(self, data_dict, modality_name, output_dict):
        ...
        if modality_name == "protocol":
            feature = eval(f"self.encoder_{self.protocol_modality}")(
                data_dict, "m0", False
            )
            return feature
