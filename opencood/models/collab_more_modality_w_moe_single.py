...
class CollabMoreModalityWMoESingle(nn.Module):

    def __init__(self, args):
        super(CollabMoreModalityWMoESingle, self).__init__()
    ...
    def forward(self, data_dict, show_bev=False):
        ...
        if len(set(batch_ego_modality_list)) == 1:
            ...
            modality_name = ego_modality_name

            fused_feature, occ_outputs = eval(f"self.pyramid_backbone_{modality_name}").forward_single(all_processed_features)
            ...
            for mini_batch_idx, mini_batch_size in enumerate(record_len):
                ...
                fused_feature, occ_outputs = eval(f"self.pyramid_backbone_{ego_modality_name}").forward_single(
        ...
        output_dict.update({
            "pyramid": "single",
            "cls_preds_single": output_dict["cls_preds"],
            "reg_preds_single": output_dict["reg_preds"],
            "dir_preds_single": output_dict["dir_preds"],
        })
