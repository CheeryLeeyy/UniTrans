...
from opencood.models.sub_modules.sparse_backbone_3d import VoxelBackBone8x, VoxelBackBone8xLarge, VoxelBackBone4x, VoxelBackBone8xMedium, VoxelBackBone4xMedium, VoxelBackBone4xLarge,VoxelBackBone2x
...
from opencood.models.voxel_net import CML, CML2, CML3
...
class VoxelNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args["cml_type"] == "normal":
            self.cml = CML()
        elif args["cml_type"] == "medium":
            self.cml = CML2()
        elif args["cml_type"] == "large":
            self.cml = CML3()
    ...
    def forward(self, data_dict, modality_name, multi_sensor=False):
        ...
        # print(f"vwfs.shape1:{vwfs.shape}")
        voxel_coords = torch_tensor_to_numpy(voxel_coords)

        # for N in data_dict['record_len']:
        # todo:直接从record_len获取N是不对的，因为这是所有模态加一块的总数
        # print(f"voxel_coords:{voxel_coords[:, 0]}")
        N = voxel_coords[:, 0].max().item() + 1
        ...
        # print(f"vwfs.shape2:{vwfs.shape}")
...
class PointPillar(nn.Module):
    ...
    def forward(self, data_dict, modality_name, multi_sensor=False):
        ...
        # print(f"after pillar_vfe shape:{batch_dict['pillar_features'].shape}")
        batch_dict = self.scatter(batch_dict)
        lidar_feature_2d = batch_dict["spatial_features"]  # H0, W0
        # print(f"after scatter shape:{batch_dict['spatial_features'].shape}")
...
class SECOND(nn.Module):
    def __init__(self, args):
        ...
        # print("lidar_range:",lidar_range)
        # print("voxel_size:",args["voxel_size"])
        # print("grid_size:",grid_size)
        self.vfe = MeanVFE(args["mean_vfe"], args["mean_vfe"]["num_point_features"])
        if "spconv_large" in args:  # 如果存在大模型配置
            spconv_cfg = args["spconv_large"]
            spconv_block_class = VoxelBackBone8xLarge  # 假设已定义大模型类
        elif "spconv_4x" in args:  # 4x
            spconv_cfg = args["spconv_4x"]
            spconv_block_class = VoxelBackBone4x
        elif "spconv_medium" in args:  # medium
            spconv_cfg = args["spconv_medium"]
            spconv_block_class = VoxelBackBone8xMedium
        elif "spconv_4x_medium" in args:
            spconv_cfg = args["spconv_4x_medium"]
            spconv_block_class = VoxelBackBone4xMedium
        elif "spconv_4x_large" in args:
            spconv_cfg = args["spconv_4x_large"]
            spconv_block_class = VoxelBackBone4xLarge
        elif "spconv_2x" in args:  # 2x
            spconv_cfg = args["spconv_2x"]
            spconv_block_class = VoxelBackBone2x
        else:  # 默认使用原版8x
            spconv_cfg = args["spconv"]
            spconv_block_class = VoxelBackBone8x
        self.spconv_block = spconv_block_class(
            spconv_cfg, input_channels=spconv_cfg["num_features_in"], grid_size=grid_size
        )
    ...
    def forward(self, data_dict, modality_name, multi_sensor=False):
        ...
        # print(f"vfe:{batch_dict['voxel_features'].shape}")
        batch_dict = self.spconv_block(batch_dict)
        # print(f"spconv_block:{batch_dict['encoded_spconv_tensor'].dense().shape}")
        batch_dict = self.map_to_bev(batch_dict)
        # print(f"map_to_bev:{batch_dict['spatial_features'].shape}")
