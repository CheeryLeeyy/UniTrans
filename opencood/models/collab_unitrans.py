...
class CollabUniTrans(nn.Module):

    def __init__(self, args):
        super(CollabUniTrans, self).__init__()
        ...
        for modality_name in self.all_modality_name_list:
            ...
            backbone = getattr(self, f"backbone_{modality_name}")
            aligner = getattr(self, f"aligner_{modality_name}")

            enc = sum(p.numel() for p in encoder.parameters())
            bkb = sum(p.numel() for p in backbone.parameters())
            alg = sum(p.numel() for p in aligner.parameters())

            # print(f"[{modality_name}] encoder  : {fmt_params(enc)} ({enc:,})")
            # print(f"[{modality_name}] backbone : {fmt_params(bkb)} ({bkb:,})")
            # print(f"[{modality_name}] aligner  : {fmt_params(alg)} ({alg:,})")
            print(f"[{modality_name}] sum      : {fmt_params(enc + bkb + alg)} ({enc + bkb + alg:,})")
        ...
        enc_params = sum(p.numel() for p in self.intrinsic_modal_encoder.parameters())
        converter_params = sum(p.numel() for p in self.converter.parameters())
        print(f"[Params] Encoder : {enc_params:,}")
        print(f"[Params] Converter : {converter_params:,}")
        print(f"[Params] All : {enc_params + converter_params:,}")
    ...
    def forward(self, data_dict, show_bev=False):
        ...
        with torch.no_grad():
            ...
            for modality_name in self.used_modality_name_list:
                ...
                code = self.intrinsic_modal_encoder(feature)  # [Ni, D], already L2-normalized
                intrinsic_code_dict[modality_name] = code
        ...
        # if self.training and sum(record_len) > 61:
        #     mapped, alphas = self.converter(feature, feature, code, code, return_alpha=True)
        #     loss = mapped.mean() * 0.0
        #     output_dict = {
        #         "skip_flag": True,
        #         "loss": loss * 0.0,
        #     }
        #     print("==" * 50)
        #     print(sum(record_len))
        #     print("SKIP!!!!!!!")
        #     print("==" * 50)
        #     return output_dict
        ...
        moe_output_dict = {
            "intrinsic_code": intrinsic_code_list,
