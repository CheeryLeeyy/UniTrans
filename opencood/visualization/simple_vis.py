...
def visualize(infer_result, 
        ...
        if method == 'bev':
            ...
            # 初始化点云颜色数组
            pcd_colors = np.ones((canvas_xy.shape[0], 3)) * 255  # 默认白色

            # 根据color_method选择颜色分配策略
            if color_method == 'cav' and pcd_cav is not None:
                # 基于车辆ID的颜色分配
                cav_ids = pcd_cav.detach().cpu().numpy()
                unique_cavs = np.unique(cav_ids)

                # 为每个车辆ID分配颜色 (最多5辆车)
                cav_color_map = {
                    0: np.array([165, 59, 158]),  # 车辆1: 紫色
                    1: np.array([38, 158, 2]),  # 车辆2: 绿色
                    2: np.array([180, 123, 51]),  # 车辆3: 棕色
                    3: np.array([30, 88, 145]),  # 车辆4: 蓝色
                    4: np.array([255, 0, 0])  # 车辆5: 红色
                }
                # cav_color_map = {
                #     0: np.array([255, 87, 51]),  # 珊瑚红
                #     1: np.array([51, 255, 87]),  # 鲜绿色
                #     2: np.array([51, 87, 255]),  # 钴蓝色
                #     3: np.array([255, 51, 246]),  # 亮紫色
                #     4: np.array([255, 243, 51])  # 柠檬黄
                # }

                # 将车辆ID映射到0-4的范围
                cav_id_to_index = {cav_id: idx for idx, cav_id in enumerate(unique_cavs)}

                for cav_id, idx in cav_id_to_index.items():
                    if idx < 5:  # 只处理前5辆车
                        mask = (cav_ids == cav_id)
                        pcd_colors[mask] = cav_color_map[idx]
                    else:
                        # 如果超过5辆车，使用灰色
                        mask = (cav_ids == cav_id)
                        pcd_colors[mask] = np.array([100, 100, 100])

                # 设置车辆颜色标签
                vehicle_color_map = cav_color_map
                render_dict = {0: 'V1', 1: "V2", 2: 'V3', 3: 'V4', 4: 'V5'}

            elif color_method == 'modality' and pcd_modality is not None:
                # 基于模态的颜色分配
                pcd_colors[:] = np.array([66, 66, 66])  # 默认灰色
                modality_map = {
                    1: np.array([165, 59, 158]),  # 模态1: 紫色
                    2: np.array([38, 158, 2]),  # 模态2: 绿色
                    3: np.array([180, 123, 51]),  # 模态3: 棕色
                    4: np.array([30, 88, 145])  # 模态4: 蓝色
                }
                modality_values = pcd_modality.detach().cpu().numpy()

                for modality, color in modality_map.items():
                    mask = (modality_values == modality)
                    pcd_colors[mask] = color

                # 设置模态颜色标签
                vehicle_color_map = modality_map
                render_dict = {1: 'M1', 2: "M2", 3: 'M3', 4: 'M4'}
            else:
                # 没有提供任何ID信息，使用统一的灰色
                pcd_colors[:] = np.array([150, 150, 150])
                vehicle_color_map = {}
                render_dict = {}
            ...
            if infer_result is not None:
                # 绘制真值框和预测框（保持不变）
                if gt_box_tensor is not None:
                    canvas.draw_boxes(gt_box_np, colors=(0, 185, 0), texts=None)
                ...
                # 处理代理车辆
                agent_modality_list = infer_result.get("agent_modality_list", None)
                agent_cav_id_list = infer_result.get("agent_cav_id_list", None)  # 新增：获取车辆ID列表
                cav_box_np = infer_result.get("cav_box_np", None)
                ...
                if agent_modality_list is not None or agent_cav_id_list is not None:
                    cav_box_np = copy.deepcopy(cav_box_np)
                    num_agents = len(agent_modality_list) if agent_modality_list is not None else len(agent_cav_id_list)

                    for i in range(num_agents):
                        # 确定代理ID（优先使用车辆ID）
                        if agent_cav_id_list is not None and i < len(agent_cav_id_list):
                            agent_id = agent_cav_id_list[i]
                        elif agent_modality_list is not None and i < len(agent_modality_list):
                            agent_id = agent_modality_list[i]
                        else:
                            agent_id = i

                        # 确定颜色
                        if color_method == 'cav' and agent_id in vehicle_color_map:
                            color = vehicle_color_map[agent_id]
                        elif agent_id in vehicle_color_map:  # 模态模式
                            color = vehicle_color_map[agent_id]
                        else:
                            color = (66, 66, 66)  # 默认灰色

                        # 确定标签文本
                        if agent_id in render_dict:
                            label = render_dict[agent_id]
                        else:
                            label = f"A{i + 1}"
