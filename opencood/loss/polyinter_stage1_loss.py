...
class PolyInterStage1Loss(nn.Module):
    def __init__(self, args):
        super(PolyInterStage1Loss, self).__init__()
        ...
        self.loss_dict = {}
        self.cross_loss = torch.nn.CrossEntropyLoss(label_smoothing=0.1)
    ...
    def forward(self, output_dict, target_dict, suffix="", val=False):
        ...
        # specific_loss
        N = output_dict['out_q'].size(0)
        out_q, out_k = output_dict['out_q'].reshape(N,-1), output_dict['out_k'].reshape(N,-1)
        out = torch.cat([out_q, out_k], dim=0)
        target_q = torch.ones(out_q.size(0), dtype=torch.long, device=out_q.device)
        target_k = torch.zeros(out_k.size(0), dtype=torch.long, device=out_k.device)
        target = torch.cat([target_q, target_k], dim=0)
        
        
        specific_loss = self.cross_loss(out, target)
        total_loss += 0.05*specific_loss
        
        
        # style loss
        mean_q, mean_k, std_q, std_k = output_dict['mean_q'],output_dict['mean_k'],output_dict['std_q'],output_dict['std_k']
        mean_k2, mean_k3, std_k2, std_k3 = output_dict["mean_k2"],output_dict["mean_k3"],output_dict["std_k2"],output_dict["std_k3"]
        N = mean_q.size(0)
        style_loss = (torch.norm(mean_q - mean_k, p=2) + torch.norm(std_q - std_k, p=2)) / N \
                        + (torch.norm(mean_q - mean_k2, p=2) + torch.norm(std_q - std_k2, p=2)) / N \
                            + (torch.norm(mean_q - mean_k3, p=2) + torch.norm(std_q - std_k3, p=2)) / N
        total_loss += 0.5*style_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, pbar = None, suffix="", iter=None):
        """
        ...
        reg_loss_key = self.loss_dict.get('reg_loss_key', 0)
        cls_loss_key = self.loss_dict.get('cls_loss_key', 0)
        specific_loss = self.loss_dict.get('specific_loss', 0)
        style_loss = self.loss_dict.get('style_loss', 0)
        ...
        if not writer is None:
            ...
            #                 epoch*batch_len + batch_id)
            writer.add_scalar('Regression_loss_key'+suffix, reg_loss_key,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Confidence_loss_key'+suffix, cls_loss_key,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Specific_loss'+suffix, specific_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Style_loss'+suffix, style_loss,
                            epoch*batch_len + batch_id)
